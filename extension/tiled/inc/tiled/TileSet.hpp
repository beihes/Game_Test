...
namespace tiled {
    ...
    class TileSet {
    public:
        TileSet(uint32_t firstgid, std::string_view source);
        ~TileSet();
    public:
        bool Parse();
        void Print() const;
    public:
        uint32_t Get_FirstGid() const { return this->firstgid_; }
        uint32_t Get_EndGid() const { return this->firstgid_ + this->tilecount_; }
