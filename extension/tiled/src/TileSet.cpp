...
namespace tiled {
    TileSet::TileSet(uint32_t firstgid, std::string_view source)
    ...
    std::string TileSet::Get_ImagePath() {
        std::filesystem::path sourcePath_(this->source_);
        return std::move((sourcePath_.parent_path() / this->image_).string());
    }
